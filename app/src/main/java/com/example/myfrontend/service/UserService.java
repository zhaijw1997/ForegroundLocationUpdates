...
public class UserService {
    public static boolean signIn(String name, String password) {
        MyThread myThread = new MyThread("http://192.168.0.63:8080/Servlet_server/SignIn",name,password);
        try
        {
            myThread.start();
            myThread.join();
    ...
    public static boolean signUp(String name, String password) {
        MyThread myThread = new MyThread("http://192.168.0.63:8080/Servlet_server/SignUp",name,password);
        try
        {
            myThread.start();
            myThread.join();
